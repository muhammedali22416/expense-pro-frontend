...
import { X, Save, CreditCard, Calendar, Type, AlertTriangle } from 'lucide-react';

// --- EDIT MODAL ---
export const EditExpenseModal = ({ isOpen, onClose, data, onSave, onChange }) => {
        ...
        >
          <div className="p-6 sm:p-10">
            <div className="flex justify-between items-start mb-8">
              <div>
                <h3 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Edit Expense</h3>
                <p className="text-slate-500 text-sm mt-1">Update your record details</p>
              </div>
              <button onClick={onClose} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 hover:text-rose-500 transition-all">
                <X size={20}/>
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div className="group">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1 block mb-2">Description</label>
                <div className="relative">
                  <Type className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  {/* UPDATE: title -> expense_title */}
                  <input type="text" value={data.expense_title || ''} onChange={(e) => onChange({...data, expense_title: e.target.value})} 
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl font-medium dark:text-white outline-none border-2 border-transparent focus:border-indigo-500 transition-all" />
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1 block mb-2">Amount (Price)</label>
                  <div className="relative">
                    <CreditCard className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                    {/* UPDATE: amount -> price */}
                    <input type="number" value={data.price || ''} onChange={(e) => onChange({...data, price: e.target.value})} 
                      className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl font-medium dark:text-white outline-none border-2 border-transparent focus:border-indigo-500 transition-all" />
                  </div>
                </div>
                <div>
              ...
              <button onClick={onSave} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-5 rounded-2xl font-bold shadow-lg shadow-indigo-200 dark:shadow-none transition-all flex items-center justify-center gap-3 active:scale-[0.98]">
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      >
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8 font-medium">
          This will permanently delete <br/> <span className="text-rose-500 font-bold block mt-1">"{itemName}"</span>
        </p>
